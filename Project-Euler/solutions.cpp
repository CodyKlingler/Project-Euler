...
*/
int pe19() {
    const char* days[] = { "mon", "tues", "wed", "thur", "fri", "sat", "sun" }; //just used to print/debug
    const char* months[] = { "jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec" };//just used to print/debug
    ...
    int year = 1900;
    int month = 0;
    int day = 0; //curent day of the week of the first month, starting with monday = 0.

    while (year <= 2000) {
        bool isLeapYear = (!(year % 4) && (year % 100)) || (!(year % 100) && !(year % 400));
        for (int month = 0; month < 12; month++) {  //for every month in the year. `day` will always be the weekday of the first month
            if (day == 6 && year >= 1901) {         //check if today is a sunday and in the range 1901 to 2000
                sundayMonths++;
            }
            int daysThisMonth = (isLeapYear && month == 1) ? daysInMonth[month] + 1 : daysInMonth[month]; //find how many days are in the month
                                                                                                          //if it's february and it's a leapyear then there is an extra day
            day = (day + daysThisMonth) % 7; //calculate  the first day of the next month
        }
        year++;
    }
    return sundayMonths;
...
int pe20() {
    ...
    for (int currentTerm = 1; currentTerm <= 100; currentTerm++) { //think of how you learned to multiply big numbers as a kid
        for (int currentDigit = mostSignificantDigit; currentDigit >= 0; currentDigit--) { //multiply each digit,product[d], by i
            int currentProduct = currentTerm * product[currentDigit];    //current product holds sub-product, or the current digit multiplied by i
            product[currentDigit] = (short)(currentProduct % 10);  //the new number digit in this position is always the last digit in the current product
            currentProduct /= 10;
            int tensPlaceOffset;
            for (tensPlaceOffset = 1; currentProduct; tensPlaceOffset++, currentProduct /= 10) { // add the current product to the other digits places
                currentProduct += product[currentDigit + tensPlaceOffset];
                product[currentDigit + tensPlaceOffset] = (short)(currentProduct % 10);
            }
            if (currentDigit + tensPlaceOffset - 1 > mostSignificantDigit) //recalculate most significant digit
                mostSignificantDigit = currentDigit + tensPlaceOffset;
        }
    }
    int sum = 0;
